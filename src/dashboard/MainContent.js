...
export default function MainContent() {
  ...
  useEffect(() => {
    ...
    if (savedTotalSpent) setTotalSpent(JSON.parse(savedTotalSpent));
    if (savedDailyLimit) setDailyLimit(JSON.parse(savedDailyLimit));
    if (savedExpense) setExpense(JSON.parse(savedExpense));
    if (savedCategoricalSpend) setCategoricalSpend(JSON.parse(savedCategoricalSpend));
  }, []);
